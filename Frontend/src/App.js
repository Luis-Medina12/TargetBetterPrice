...
const App = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if(false){
      ...
      const original_data = JSON.parse(JSON.stringify(exampleResp));
      setResponseData([]);
      
      for(const store of exampleResp.locations)
      {
        const info = new OutputData(store.address.address_line1, 
          store.location_names[0].name, expProduct.product.price.current_retail, store.distance, 
        ...
        setResponseData((responseData) => ([...responseData, info]));
      };
      navigator.geolocation.getCurrentPosition(({coords: {latitude, longitude} }) =>{
        setHome({lat: latitude, lng: longitude});
      })
    }
  };
